...
//extern rgb_led_color led_color;
...
void dance(void){
	desired_speed = (int)(0.6 * (float)MOTOR_SPEED_LIMIT);
...
void manage_robot(void){
	if((mean_red >= 7 && mean_red <= 9 &&
		mean_green >= 11 && mean_green <= 13 &&
		mean_blue >= 7 && mean_blue <= 9) || get_detect_peach() == true) {
		set_detect_peach(true);
		set_led_color(pink);
		/*set_led(LED1, 1);
		set_led(LED3, 0);
		set_led(LED5, 0);
		...
		if(get_prox(FRONT_RIGHT17) > 150 && get_is_turning() == false){
			set_led_color(white);
			/*set_led(LED1, 0);
			set_led(LED3, 0);
			set_led(LED5, 1);
	...
	} else if(mean_red >= 0 && mean_red <= 2 &&
			mean_green >= 5 && mean_green <= 6 &&
			mean_blue >= 5 && mean_blue <= 6){
		...
		set_led(LED5, 0);
		set_led(LED7, 1);*/
		desired_speed = (int)(0.4 * (float)MOTOR_SPEED_LIMIT);
	}
}
